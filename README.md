# stage_6_developer_roadmap 
# JavaScript

https://learn.javascript.ru/
https://metanit.com/web/javascript/2.9.php
_____

* ~~Привет, мир!~~
* ~~Внешние скрипты~~
* ~~Структура кода~~
* ~~Комментарии~~
* ~~Переменные~~
* ~~Константы~~
* ~~Типы данные~~
  - Число
  - BigInt `` const bigInt = 1234567890123456789012345678901234567890n;``
  - Строка
  - Булевый (логический) тип
  - Значение «null»
  - Значение «undefined»
  - Объекты и символы (Понимание, что такое есть)
  - Оператор typeof
* ~~alert, prompt, confirm~~
* ~~Строковое преобразование~~
* ~~Численное преобразование~~
* ~~Логическое преобразование~~
* ~~Базовые операторы, математика~~
  - «унарный», «бинарный», «операнд»
  - Математика
  - Взятие остатка %
  - Возведение в степень **
  - Сложение строк при помощи бинарного +
  - +"2" - То же самое что и Number("2")
  - Присваивание = возвращает значение ```let c = 3 - (a = b + 1);```
  - Присваивание по цепочке
  - Сокращённая арифметика с присваиванием
  - Инкремент/декремент
  - Побитовые операторы (пока пропускаю)
* ~~Операторы сравнения~~
* ~~Условное ветвление: if, '?'~~
* ~~Логические операторы~~
* ~~Операторы нулевого слияния и присваивания: '??', '??='~~
* ~~Циклы while и for~~
* ~~Конструкция "switch"~~
* ~~Функции~~
* ~~Function Expression~~
* ~~Стрелочные функции, основы~~
* ~~Замыкания~~
* ~~Функции IIFE~~
* ~~Рекурсивные функции~~
* ~~Переопределение функций~~
* ~~Передача параметров по значению и по ссылке~~
* ~~ООП~~
  - ~~ООП. Классы~~
  - ~~ООП. Приватные поля и методы~~
  - ~~ООП. Свойства и методы доступа~~
  - ~~ООП. Наследование + super~~
* ~~Массивы~~
  - Array.from
  - spread-оператор
  - Передача аргументов функции
  - Операции с массивами - push(), splice(), pop(), shift(), slice(), copyWithin(), indexOf(), lastIndexOf(), includes(), every(), some(), filter(), forEach(), map(), find(), findLast(), findIndex(), flat(), reduce()
  - Наследование массивов
* ~~Строки и регулярные выражения~~
  - repeat(), indexOf(), lastIndexOf(), includes(), substring(startIndex, endIndex), slice(startIndex, endIndex), toUpperCase(), toLowerCase(), startsWith()
  - Шаблоны строк
  - Объект RegExp. Регулярные выражения = .test(), .exec()
  - Синтаксис регулярных выражений
  - Квантификаторы в регулярных выражениях
  - Поиск в строке
  - Группы в регулярных выражениях
  - Именнованные группы (?<название_группы> ... )
  - Регулярные выражения в методах String = match, split, search, replace
* ~~Обработка ошибок~~
  - Конструкция try..catch..finally
  - Генерация ошибок и оператор throw
* ~~Встроенные объекты~~
  - Объект Date. Работа с датами
  - бъект Math. Математические операции
  - Объект Number
  - Символы
  - Proxy new Proxy(target, handler);
* ~~Коллекции и итераторы~~
  - Итераторы
  - Генераторы
  - Множества Set
  - Map
  - WeakSet (Для объектов)
  - WeakMap (Для объектов)
* ~~Работа с DOM~~
  - Свойства объекта document
  - Поиск элементов на веб-странице
  - Объект Node. Навигация по DOM
  - Элементы
  - Создание, добавление, замена и удаление элементов
  - Управление атрибутами элементов
  - Изменение стиля элементов
  - Создание своего элемента HTML = Добавление методов, События жизненного цикла, Добавление атрибутов, Стилизация CSS
* ~~События~~
  - Введение в обработку событий
  - Обработчики событий
  - Передача данных в обработчик события. Объект Event
  - Распространение событий
  - События мыши
  - События клавиатуры
  - Программный вызов событий
  - Определение своих событий
* ~~Работа с браузером и BOM~~
  - Browser Object Model и объект window
  - location
  - Таймеры
* Форматы JSON и XML
* Хранение данных = Куки (Параметр expires, Параметр secure), Web Storage
